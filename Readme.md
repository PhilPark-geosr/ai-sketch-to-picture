...
| 추천 | `src/components/ImageModal.tsx`, `src/components/RecommendView.tsx` | `POST {SERVER_URL}/recommend` 후 `RecommendView`로 이동 |
...
`RecommendView`는 `route.params.recommendResponse`가 없으면 `src/mock/recommend.json`을 표시합니다. 서버 연동 테스트 시 실제 응답이 params로 전달되는지 확인하세요.

## 환경변수 설정

프로젝트 루트에 `.env` 파일을 만들고, `babel.config.js`의 `react-native-dotenv` 설정(`moduleName: '@env'`, `path: '.env'`)에 맞게 값을 넣습니다. 템플릿은 [`.env.example`](./.env.example)를 참고하세요.
...
- **업로드 응답** (`/upload`): JSON `{ "image": string }` — `image`로 `PickedAsset.uri`와 `PickedAsset.base64`를 채운 뒤 `ImageModal` 표시
- **추천 응답** (`/recommend`): `RecommendResponse` (`src/types/recommend.ts`) — `status`, `prompt`, `image_url`, `results[]`

`SketchUploader`는 base64 문자열에 `data:image/png;base64,` 접두사가 있으면 제거한 뒤 multipart에 붙입니다. 카메라·갤러리 경로는 `expo-file-system` / `expo-image-picker`로 raw base64를 준비합니다.
...
- **추천 요청 시 base64 오류**: `/upload` 응답의 `image`가 URL이면 `ImageModal`의 `/recommend` 호출이 실패할 수 있습니다. 서버는 base64 PNG 또는 클라이언트가 재인코딩 가능한 형식을 반환해야 합니다.
