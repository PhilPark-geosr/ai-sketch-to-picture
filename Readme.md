...
| 추천 결과 | `src/components/RecommendView.tsx` | `Recommend` 화면, `data` 없으면 `src/mock/recommend.json` 표시 |
...
`MemoSketch` 하단 `buttonContainer`에 `CameraView`와 `RecordButton`이 나란히 배치됩니다. 둘 다 `@expo/vector-icons`의 `Ionicons` 아이콘 버튼입니다.

| 컴포넌트 | 아이콘 | 동작 |
|----------|--------|------|
| `CameraView` | `camera` (주황 배경) | `expo-image-picker`로 촬영 → base64 `PickedAsset` → `ImageModal` |
| `RecordButton` | `mic` (녹색 배경) | `AudioRecordModal.open()` 호출 |

### 음성 녹음 UI 상태

`AudioRecordModal`은 ref로 `open()` / `close()`를 노출합니다.

1. **녹음 중** (`isRecording === true`): `Ionicons` 아이콘 버튼
   - `trash-outline` — 녹음 취소·모달 닫기
   - `pause` / `play` — 일시정지·재개
   - `checkmark` — 녹음 완료(정지 후 URI 저장)
2. **완료 후**: 재생 미리보기(텍스트 버튼), 저장(STT 업로드) 또는 취소(텍스트 버튼)
...
|------------|----------------|-----------|
| `/upload` | `sketch` (PNG) | `prompt` — `MemoSketch` 텍스트 입력값 (`text` state) |
| `/recommend` | `image` (PNG) | `prompt` — `ImageModal` 텍스트 입력값 |
...
`SketchUploader`는 base64 문자열 앞의 `data:image/png;base64,` 접두사가 있으면 제거한 뒤 업로드합니다. `prompt` 인자를 생략하면 기본값 `"realistic image"`가 사용되지만, `MemoSketch`·`ImageModal`은 각 화면의 입력값을 그대로 전달합니다(빈 문자열 가능).
...
| 갤러리 선택 | `GalleryPickerManager.pickImage` — `ImagePicker.launchImageLibraryAsync` (시스템 권한 다이얼로그) |
