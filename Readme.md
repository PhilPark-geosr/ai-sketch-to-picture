...
`AudioRecordModal`은 ref로 `open()` / `close()`를 노출합니다. `RecordButton`·`CameraView`·녹음 모달 컨트롤은 `@expo/vector-icons`의 Ionicons 아이콘을 사용합니다.
...
- base64 문자열에 `data:image/png;base64,` 접두사가 붙어 있어도 `SketchUploader`가 제거한 뒤 업로드합니다.
...
| 갤러리 선택 | `GalleryPickerManager` — `ImagePicker.launchImageLibraryAsync()` |
